...
namespace WebApplication6.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> ExisteCuenta(int idEmpleado)
        {
            var userId = User.Identity.GetUserId();
            using (var context = new ApplicationDbContext())
            {
                var user = context.Users.Include(x=>x.Empleado).Where(x => x.Id_Empleado == idEmpleado).FirstOrDefault();
                return View(user);
            }
        }
        //
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                if (user != null)
                {
                    ...
                    using (var context = new ApplicationDbContext())
                    {
                        var emp = context.TbEmpleado.Find(user.Id_Empleado);
                        emp.FirstLogin = true;
