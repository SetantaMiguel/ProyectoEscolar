...
{   
    ...
    public class EmpleadoMateriasController : Controller
    {
        ...
        {
            auxId = IdEmpleado;
            var tbEmpleadoMaterias = db.tbEmpleadoMaterias.Include(e => e.Empleado).Include(e => e.Materias).Where(x=>x.Id_Empleado==IdEmpleado).ToList();
            return PartialView(tbEmpleadoMaterias);
        }
        ...
        public async Task<ActionResult> Create([Bind(Include = "Id_EmpleadoMateria,Id_Empleado,Id_Materia")] EmpleadoMaterias empleadoMaterias)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                return RedirectToAction("Index",new { IdEmpleado = auxId});
            }
        ...
        public async Task<ActionResult> Delete(int? id)
        {
            ...
            EmpleadoMaterias empleadoMaterias = await db.tbEmpleadoMaterias.Include(x=>x.Empleado).Include(x=>x.Materias).Where(x=>x.Id_EmpleadoMaterias==id).FirstAsync();
            if (empleadoMaterias == null)
            {
                return HttpNotFound();
            }
